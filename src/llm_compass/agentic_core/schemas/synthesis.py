...
class ScoreCI(BaseModel):
    """Confidence interval for a tier-specific blended score.

    Propagated linearly from PerformanceCI via:
        low  = perf_w * perf.low  + (1 - perf_w) * cost_index
        high = perf_w * perf.high + (1 - perf_w) * cost_index

    For Top Performance (perf_w=1.0) this equals the raw PerformanceCI.
    For Budget (perf_w=0.2) the interval is 5x narrower.
    """
...
class TierTableRow(BaseModel):
    ...
    score_ci: Optional[ScoreCI] = None  # CI propagated from PerformanceCI through blended formula
...
class SynthesisLLMOutput(BaseModel):
    ...
    task_summary: str = Field(description="1-2 sentence rephrasing of the user's intended task.")
