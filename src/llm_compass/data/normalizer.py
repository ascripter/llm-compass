...
DATE_PATTERNS = [
    ...
    (r'\((\d{1,2})/(\d{2,4})\)', lambda m: f"{m.group(2) if len(m.group(2))==4 else '20'+m.group(2)}-{m.group(1).zfill(2)}"),
]

def normalize_model(raw_name: str):
    if not raw_name or pd.isna(raw_name): return "n/a", "n/a", "unknown"
    ...
    if re.search(r'\bo\d|deepseek[- ]?r1', raw_name, re.IGNORECASE):
        ...
        for pat, var in [(r'\bthinking\b', 'thinking'), (r'\binstruct\b', 'instruct'), (r'\bpreview\b', 'preview')]:
    ...
    if version_val: name = name.replace(version_val, ' ')
    ...
    full_id = re.sub(r'-standard$', '', re.sub(r'-+', '-', '-'.join([str(p) for p in full_parts if p]).lower()))
