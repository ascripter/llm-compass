...
_DATE_PAREN_RE = re.compile(r"^\s*(\d{4}-\d{2}-\d{2}|\d{1,2}/\d{2,4}|\d{4}-\d{2}|\d{2,8})\s*$")
...
def _tokenize(raw: str) -> list[str]:
    ...
    for raw_tok in re.split(r"[\s\-_/:\\+]+", main):
        ...
        has_version_boundary = any(
            (_is_ver(sub[i]) and _is_txt(sub[i + 1])) or (_is_txt(sub[i]) and _is_ver(sub[i + 1]))
            for i in range(len(sub) - 1)
        )
        has_camel_split = len(sub) > 1 and all(_is_txt(t) for t in sub) and len(raw_tok) > 4
...
_DATE_PATTERNS = [
    ...
    (r"[Pp]review\s+(\d{2})-(\d{2})\b", lambda m: f"2025-{m.group(1)}-{m.group(2)}"),
    (
        r"[\(\-](\d{2})(\d{2})\)?(?:\b|$)",
        lambda m: f"20{m.group(1)}-{m.group(2)}" if int(m.group(1)) >= 20 else None,
    ),
    (r"\((\d{1,2})/(\d{2})\)", lambda m: f"20{m.group(2)}-{m.group(1).zfill(2)}"),
    (r"\((\d{1,2})/(\d{4})\)", lambda m: f"{m.group(2)}-{m.group(1).zfill(2)}"),
    (r"\b(\d{4})-(\d{2})\b", lambda m: f"{m.group(1)}-{m.group(2)}"),
]
...
def _extract_size(tokens: list[str]) -> tuple[Optional[str], list[str]]:
    first_tier_idx = next((i for i, t in enumerate(tokens) if _TIER_SIZE_RE.match(t)), None)
    tier_is_subfamily = (
    ...
    for i, tok in enumerate(tokens):
        ...
        if _TIER_SIZE_RE.match(tok) and tier_is_subfamily:
            continue
        _FUSED_MOE_RE = re.compile(r"^(\d+(?:\.\d+)?[tbmk])(a\d+(?:\.\d+)?[tbmk])$", re.I)
...
def normalize(raw: str) -> dict:
    ...
    tokens = [t for t in tokens if not _is_date_token(t) and t not in _NOISE_TOKENS]
    ...
    if is_o:
        size = None
        tokens = [t for t in tokens if not _TIER_SIZE_RE.match(t) and not _MOE_FRAG_RE.match(t)]
    else:
    ...
    def _clean(p):
        return re.sub(r"-{2,}", "-", "-".join(p).lower()).strip("-")
